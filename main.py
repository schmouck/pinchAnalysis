...
def main():
    ...
    #cold_hc = [f+1500 for f in cold_hc]


    x = fmin(pinchdar, x0=100, args=(cold_hc, cold_temp, hot_hc, hot_temp, 0))
...
def filling_bp_for_hc(hot_hc, hot_bp, cold_hc, cold_bp):
    ...
    for hc in max_it:
        if hc not in hot_hc:
            hc_index = clist(hot_hc[:], [hc]).index(hc)
        ...
        if hc not in cold_hc:
            hc_index = clist(cold_hc[:], [hc]).index(hc)
...
def pinchdar(d_hc, cold_hc, cold_temp, hot_hc, hot_temp, obj):
    cold_hc = [hc + d_hc for hc in cold_hc]
    cold_hc, cold_temp, hot_hc, hot_temp = filling_bp_for_hc(hot_hc, hot_temp, cold_hc, cold_temp)
    return obj - min_dist(cold_hc, cold_temp, hot_hc, hot_temp)
